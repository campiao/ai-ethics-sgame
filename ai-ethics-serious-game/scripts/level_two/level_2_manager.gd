...
@onready var end_exam_label: Label = $EndExamination
...
var image_analysis_res = [
	"You got all the bonus points. Good Job!",
	"Wasted all your attempts. Nice work tho! "
]
...
func _process(_delta: float) -> void:
	...
	#	return
		...
		elif end_exam_label.visible_ratio < 1.0:
			if not is_audio_playing:
				$SFX/RobotTalk.play()
				is_audio_playing = true
			end_exam_label.visible_ratio += \
			1.0/end_exam_label.text.length()/text_update_speed
		else:
			if is_audio_playing:
				$SFX/RobotTalk.stop()
...
func set_examination_text(index : int) -> void:
	end_exam_label.visible_ratio = 0.0;
	end_exam_label.text = image_analysis_res[index];
